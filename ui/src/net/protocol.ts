// @generated by protobuf-ts 2.2.2 with parameter client_none
...
import { derived, type Readable } from "svelte/store";export interface RpcTransport {
...
export class ConfigServiceClient {
    ...
    zigbee2MQTT(input: Zigbee2MQTTConfig): Readable<ConfigResult> {
        return derived(
            this.transport.call(
                0x0, 
                Zigbee2MQTTConfig.toBinary(input)
            ), 
            b => b ? ConfigResult.fromBinary(b) : null
        );
    }

    listen(input: Sensor): Readable<Sensor> {
        return derived(
            this.transport.call(
                0x1, 
                Sensor.toBinary(input)
            ), 
            b => b ? Sensor.fromBinary(b) : null
        );
    }
}
