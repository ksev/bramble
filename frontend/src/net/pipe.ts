...
class SocketTransport implements RpcTransport {
    ...
    async unary(serviceId: number, callId: number, input: Uint8Array): Promise<Uint8Array> {
        const prefix = new Uint8Array(7); // Prefix is 5 bytes
        ...
        view.setUint16(3, serviceId, false);
        view.setUint16(5, callId, false); // Which call we want to use
            ...
            this.channelResponse(chanId, (action, data) => {
                ...
                switch (action) {
                    ...
                    case PipeAction.Error:
                        ...
                        break;
                    default:
                        reject(new Error(`Channel ${chanId} expected Response action got action ${action}`));
                }
            })
        });
    }
 ...
 * Rome RPC service
 */
export const zigbee2MqttService: SvelteStore<null | Zigbee2MqttServiceClient> = derived(socket, (sock, set) => {
    ...
    set(new Zigbee2MqttServiceClient(transport));
